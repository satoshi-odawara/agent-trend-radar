...
- [ ] #16 has_observability_depのsegment適用範囲の見直し
...
**対応案(たたき台、着手時に確定する)**
- (a) `has_observability_dep`をtoolセグメント限定の項目として扱う
      (SPEC.mdのデータスキーマ・レポート出力の解釈注記を更新)
- (b) adopterセグメント向けに別の可観測性指標(例: APM/エラー
      トラッキングツールへの依存)を新設する
- (c) 現状維持しつつ、レポート・記事化の際に「adopterでは意味を
      持たない項目」と明記する
